...
const transport = nodemailer.createTransport({
  ...
  secure: true,
  auth: {
    user: "itreport@waternels.com",
    pass: "HLCwiift7mVFyqT",
  },
});
...
exports.sendPasswordResetEmail = async ({
  ...
  const mailOptions = {
    from: process.env.USERNAME || "itreport@waternels.com",
    to: email,
    subject: emailsSubject,
    text: emailText,
    html: `<p>
...
exports.sendPaymentEmail = async ({
  ...
  const mailOptions = {
    from: process.env.USERNAME || "cool@cool.com",
    to: email,
    subject: emailsSubject,
    text: emailText,
    html: `<p>
    ...
    <div style="text-align:center">Claim your payment by clicking on the following link .
        <a style="padding:8px 16px; display: block; background: dodgerblue; max-width: 300px; margin: 12px auto 0; text-align: center; border-radius: 2px; text-decoration: none; color: white;" href='https://127.0.0.1:3000/verify-payment?token=${token}'>Claim payment</a>
